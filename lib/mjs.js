...
function parseArgs(argv) {
  ...
  var minimistOpts = {
    ...
    boolean: ['debug', 'help', 'map', 'map-embed', 'map-source', 'color', 'es5-generators',
  ...
  } else if (sources.indexOf('<stdin>') !== -1) {
...
function showHelpAndExit(error, exitcode) {
  ...
  stdout([
    ...
    '          --parallel[=n]  Parallelize compilationCompile multiple files [default: off]',
...
function main(argv) {
  ...
  console.log(cfg.options.parallel, parseInt(cfg.options.parallel));
  var poolSize = parseInt(cfg.options.parallel, 10) || require('os').cpus().length;
  poolSize = Math.min(10 /*sources*/, poolSize);

  var multiplier = 1;

  if (cfg.options.fork) {
    var count = poolSize * multiplier;

    var childs = [];
    for (var i=1; i<=poolSize; i++) {
      console.log('Spawn', i);
      // var child = child_process.fork('./lib/metajs/compile');
      // childs.push(child);
      // child.on('message', function(m) {
      //   console.log('PARENT got message:', m);
      //   if (--count === 0) {
      //     childs.forEach(function (c) { c.kill(); });
      //   }
      // });
      // for (var j=0; j<multiplier; j++)
      //   child.send(cfg.config);
    }
  } else {
    for (var i=1; i<=poolSize*multiplier; i++) {
      // console.log(config);
      Meta.setOptions(cfg.config);

      var compiler = Meta.compilerFromFile(cfg.config.source);
      compiler.compile();

      console.log(compiler.errors.length);
    }
