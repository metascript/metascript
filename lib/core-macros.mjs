      ...
      moduleName.newMeta
        `
          try
            ...
            ; location. Then we perform the actual load from the resolved path.
            var require-like = require 'require-like'
            var meta-require = require-like(ast.compiler.parser.source)
            var resolved-path = meta-require.resolve(~`(moduleName.newValue module-string))
            ; Now let's actually load the module from it's own path
            meta-require = require-like(resolved-path)
            (meta-require resolved-path) (ast)
          catch (var e)
