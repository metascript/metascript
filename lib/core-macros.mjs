...
#keepmacro #r
''' Generates a literal regular expression
...
'''
  ...
  HIGH
  expand: (expr) ->
    if (!expr.value?() || typeof expr.get-simple-value() != 'string') do
      expr.error 'Expected a string literal'
      return undefined

    ; Parse global mode-modifier flags
    var
      flags = ''
      literal = expr.val.replace
        new RegExp('^\s*\(\?([A-Za-z]+)\)\s*|\s*\(\?([A-Za-z]+)\)\s*$')
        (m0, m1, m2) ->
          flags += m1 || m2
          ''

    if (expr.get('quote') && expr.get('quote').length > 1 && flags.index-of('x') == -1) do
      flags += 'x'

    ; Use XRegExp as compiler to support "x" mode-modifier
    try
      var XRegExp = require('xregexp').XRegExp
      literal = XRegExp(literal, flags).toString()
    catch (var e)
      expr.error e.message
      return undefined

    var code = ` (#external eval)( ~` expr.new-value(literal) )
    code.set('js', literal)
    code
