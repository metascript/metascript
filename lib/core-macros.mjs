...
#keepmacro #metaimport
  ...
  preExpand: (moduleName) ->
    var metaRequire = moduleExpression ->
      moduleName.new-meta
        `
          try
            (require do ~`moduleExpression) (ast)
          catch (var e)
            ast.error('Error importing module: ' + e.toString())
          null

    if (moduleName.tag?()) do
      var moduleString =
        moduleName.newValue
          moduleName.getTag ()
      metaRequire
            ...
            var iterate = dir ->
              if (fs.existsSync(path.join(dir, 'package.json')))
                ...
                if (next == dir)
                  ...
                  iterate next
            iterate(path.dirname file)
          path.join(packageRoot(ast.compiler.parser.source), 'node_modules', ~`moduleString)
    else do
      ...
      if (typeof moduleString != 'string') do
        ...
        metaRequire
          `
            ...
            (path.resolve (sourceDir, ~` (moduleName.newValue moduleString)))
