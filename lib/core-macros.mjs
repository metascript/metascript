...
#keepmacro '\\'
'''
  Lambda declaration, anything on the right is wrapped in a function.
  If there is a callable target on the left it'll be given as last
  param to it.
'''
  ...
  LOW
  expand: (lhs, body) ->
    ; Perhaps this is too hacky, if the body starts with an empty placeholder we assume we want
    ; to fill it in with first argument
    if (body.count > 1 && body.at(0).placeholder?() && body.at(0).getSimpleValue() == null) do
      body.at(0).replaceWith(body.at(0).new-placeholder '#it')
    if (body.count > 1 && body.at(-1).placeholder?() && body.at(-1).getSimpleValue() == null) do
      body.at(-1).replaceWith(body.at(-1).new-placeholder '#it')

    if (typeof body == 'undefined' || (body.isPlaceholder() && body.getSimpleValue() == null))
      ; Empty lambda serves as identity (TODO: Perhaps it's better to raise an error)
      body = ast.newPlaceholder('#it')

    ; TODO: Make it work with #-> instead of duplicated the code
    body = body.as-tuple()
    ...
    if (lhs.placeholder?())
      ...
      ; Insert the callback as last param
      lhs.at(1).asTuple().push(lambda)
      lhs
    else
      ` (~` lhs) (~` lambda)
