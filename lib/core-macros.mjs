...
#keepmacro #metaimport
  arity: unary
  precedence: LOW
  postCombine: module-names ->
    var module-strings = module-names.as-tuple().map
      module-name ->
        var module-string = module-name.get-simple-value()
        if (typeof module-string != 'string') do
          module-name.error 'Invalid module name'
          null
        else
          module-name.new-value module-string

    module-names.newMeta
      `
        try
          ; To support loading code from the compile unit source tree and
          ; not from the compiler's one we have to work around Node's default
          ; behaviour.
          ; First we resolve the module to be loaded from the source code
          ; location.
          var require-like = require 'require-like'
          var meta-require = require-like ast.compiler.parser.source
          ; Now let's actually load the modules from their own path
          [~`module-strings].for-each
            m ->
              var resolved-path = meta-require.resolve m
              var module-require = require-like resolved-path
              (module-require resolved-path) ast
