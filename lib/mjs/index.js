...
function main(argv, done) {
  ...
  if (ctx.options.parallel && !poolSize) {
    ...
    poolSize = Math.min(
      Math.round(ctx.sources.length / 3), 
    ...
    pool.send(action, function (result) {
      ...
      if (--waiting === 0) {        
        ...
        if (ctx.options['verbose']) {
          ...
          var succesfull = '';
          if (ctx.sources.length - failed > 0) {
            succesfull = 'Successfully generated ' + ctx.sources.length + ' files in ' + prettyTime + '.';
          }

          if (0 < errors.length) {
            stderr(errors.length + ' error(s) found. ' + succesfull);
          } else {
            stderr(succesfull);
          }
        }

        done(Math.min(errors.length, 255));
      }
    });
  });
}
