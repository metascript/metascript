...
function Meta() {
  ...
  meta.addArity = function (needsLeft, canHaveLeft, needsRight,
        canHaveRight, minOperandCount, maxOperandCount, isSequence,
        key, name) {
    var arity = {
      needsLeft: needsLeft,
  ...
  meta.addArity(false, false, false, false, 0, 0, true,  'aritySequence', 'sequence');
  meta.addArity(false, false, true,  true,  2, 2, false, 'arityBinary',   'binaryKeyword');
  meta.addArity(false, false, false, false, 0, 0, false, 'arityZero',     'zero');
  meta.addArity(false, false, false, true,  0, 1, false, 'arityOptional', 'optional');
  meta.addArity(false, false, true,  true,  1, 1, false, 'arityUnary',    'unary');
  meta.addArity(true,  true,  true,  true,  2, 2, false, 'arityBinary',   'binary');
  meta.addArity(true,  true,  false, false, 1, 1, false, 'arityPostfix',  'post');

  meta.addArity(false, true,  true,  true,  1, 2, false, 'aritySum',           '<sum>');
  meta.addArity(false, true,  false, true,  1, 1, false, 'arityIncrement',     '<inc>');
  meta.addArity(false, false, true,  true,  1, 1, false, 'arityPreIncrement',  '<preinc>');
  meta.addArity(true,  true,  false, false, 1, 1, false, 'arityPostIncrement', '<postinc>');
  ...
  meta.SymbolData = function (arity, precedence, options) {
    ...
    this.resolve = typeof options.resolve !== 'undefined' ? options.resolve : function (expr, vTagMap) {
              ...
              if (!expr.isResolved) {
                if (expr.isTagDeclaration()) {
                  if (meta.isReserved(name)) {
                    ...
                    previousDeclaration = vTagMap.get(name);
  ...
  meta.describeTagSymbol = function (symbol, toConstant, toDeclaration, toVirtual) {
  ...
  meta.KEY_ROOT_SCOPE.set('.', new meta.Symbol('.', 'builtin',
    new meta.SymbolData(meta.arityBinary, 'MEMBER', {
      ...
      resolve: function (expr, vTagMap) {
        ...
        if (!expr.args[1].isName()) {
          expr.args[1].error('Property name expexted');
        }
      },
      isAssignable: true
    })));
  ...
  function Compiler(r) {
    ...
    Expr.prototype.forEachRecursive = function (f, includeThis, postOrder) {
      ...
      for (var i = 0; i < this.args.length; i++) {
        this.argAt(i).forEachRecursive(f, true, postOrder);
      }
      if (includeThis && postOrder) {
        f(this);
      }
    ...
    Expr.prototype.resolvedVal = function () {
      if (this.isVirtualTag() && !this.isTagDeclaration()) {
        if (this.val === null) {
          return null;
    ...
    Expr.prototype.handleAsVirtualTag = function () {
      this.handleTag(function (tag) {
        var newSymbol = tag.sym.symbolData.toVirtual;
        if (newSymbol === null) {
                  ...
                  switch (key) {
                    ...
                    case 'fromRight':
                      if (valueString === null ||
                        ...
                        valueExpression.error('Unrecognized associativity');
                        return false;
                      }
                      fromRight = valueString === 'true' ? true : false;
                      break;
                    case 'dependsFrom':
                      if (valueString === null || meta.getArity(valueString) === null) {
                        valueExpression.error('Unrecognized dependency');
                        return false;
                      }
                      dependsFrom = valueString;
                      break;
                    case 'expand':
    ...
    Expr.prototype.expand = function () {
      var i = 0;
      ...
      while (true) {
        var result = this.sym.symbolData.expand(this, this.compiler.metaEnv);
        if (typeof result === 'undefined') {
          return true;
        } else if (result === null) {
          this.disconnect();
  ...
  Compiler.prototype.logErrors = function () {
    for (var i = 1; i < this.errors.length; i++) {
  ...
  Compiler.prototype.performPhase = function (phaseName, phaseDescription, root) {
  ...
  Compiler.prototype.expand = function () {
    return this.performPhase('expand', 'expand macros');
  };

  Compiler.prototype.resolve = function (root) {
    return this.performPhase('resolve', 'resolve symbols', root);
  };
  Compiler.prototype.checkArity = function (root) {
    return this.performPhase('checkArity', 'check arity', root);
  };

  Compiler.prototype.postExpansionPipeline = function (root) {
    this.resolve(root);
    this.checkArity(root);
  };
  ...
  Compiler.prototype.compile = function () {
    ...
    //console.log(util.inspect(jsAst, {depth: null}));
    ...
    try {
      ...
      this.root.error('Error in code generation');
      return null;
    }
  };
}
