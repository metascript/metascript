...
function Meta() {
    ...
    new meta.SymbolData(meta.aritySequence, 'NONE', {
      checkArity: function (expr) {
        for (var i = 0; i < expr.argCount(); i++) {
          ...
          if (!arg.isProperty() && !arg.isMember() && !arg.isTag()) {
            arg.error('Property definition expected');
          }
          if (arg.isProperty()) {
            arg.checkArity(0, -1, -1);
          } else {
            arg.checkArity(1, -1, -1);
          }
        }
        return 1;
      },
      codegenAsExpression: function (expr, context) {
        var members = expr.map(function (arg) {
          if (arg.isProperty()) {
            return meta.codegenObjectProperty(
              arg.at(0).val,
              arg.at(1).codegenAsExpression(context))
          } else if (arg.isMember()) {
            return meta.codegenObjectProperty(
              arg.at(1).val,
              arg.codegenAsExpression(context))
          } else {
            return meta.codegenObjectProperty(
              arg.val,
              arg.codegenAsExpression(context))
  ...
  meta.propSymbol = new meta.Symbol(':', 'builtin',
  ...
  meta.KEY_ROOT_SCOPE.set(':', meta.propSymbol);
    ...
    Expr.prototype.getObjectValue = function (transformation) {
      ...
      if (this.isObject()) {
        for (var i = 0; i < this.count; i++) {
          ...
          } else if (child.isMember() || child.isTag()) {
            var key = child.getSimpleValue();
            if (key !== null) {
              result[key] = transformation(child);
            } else {
              child.error('Invalid property tag');
              ok = false;
            }
          } else {
