...
function Meta() {
  ...
  meta.objectSymbol = new meta.Symbol('<object>', 'builtin',
    new meta.SymbolData(meta.arityZero, 'NONE', {
      ...
      resolve: function (expr) {
        ...
        meta.processDeclarations(expr.args[0], expr, false);
      }
    }));
  ...
  meta.KEY_ROOT_SCOPE.set('->', new meta.Symbol('->', 'builtin',
    new meta.SymbolData(meta.arityBinary, 'FUNCTION', {
      ...
      resolve: function (expr) {
        ...
        meta.processDeclarations(expr.args[0], expr, true);
        expr.args[1].resolve();
      },
      opensNewScope: true,
      canHostDeclarations: true
  ...
  meta.declareTag = function (tag, declaration, isArgument) {
  ...
  meta.processDeclarations = function (tags, declaration, isArgument) {
    if (tags.sym === meta.tagSymbol) {
      meta.declareTag(tags, declaration, isArgument);
    } else if (tags.sym === meta.tupleSymbol) {
      for (var i = 0; i < tags.args.length; i++) {
        var tag = tags.args[i];
        if (tag.sym === meta.tagSymbol) {
          meta.declareTag(tag, declaration, isArgument);
        } else {
  ...
  function Compiler(r) {
    ...
    Expr.prototype.checkArity = function (expectedResultCount, endArity, loopArity) {
      ...
      this.resultCount = this.sym.symbolData.checkArity(
          this, expectedResultCount, endArity, loopArity);

      if (expectedResultCount > 0 && this.resultCount !== expectedResultCount) {
