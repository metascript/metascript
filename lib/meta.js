...
function Meta() {
  ...
  function Compiler(r, options) {
    ...
    Expr.prototype.createMacro = function (
        id, arityName, precedence, dependents, expand, precombine, postcombine) {
      if (typeof id !== 'string') {
      ...
      if (!meta.isArray(dependents)) {
        this.error('Invalid symbol dependents');
        return null;
      }
      for (var i = 0; i < dependents.length; i++) {
        if (typeof dependents[i] !== 'string') {
          this.error('Invalid dependent symbol ' + dependents[i]);
          return null;
        }
        if (!this.keyScope.has(dependents[i])) {
          this.error('Undefined dependent symbol ' + dependents[i]);
          return null;
        }
      }
      if (!expand) expand = meta.undefinedFunction;
      if (typeof expand !== 'function') {
        this.error('Invalid symbol expander');
        return null;
      }
      if (!precombine) precombine = meta.undefinedFunction;
      if (typeof precombine !== 'function') {
        this.error('Invalid symbol precombiner');
        return null;
      }
      if (!postcombine) postcombine = meta.undefinedFunction;
      ...
      return new meta.Symbol(name, 'macro', data);
    };
