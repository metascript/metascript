...
function Meta() {
  ...
  meta.KEY_ROOT_SCOPE.set('loop', new meta.Symbol('loop', 'builtin',
    new meta.SymbolData(meta.arityBinaryKeyword, 'NONE', {
      checkArity: function (expr, expectedResultCount) {
        ...
        var body = expr.argAt(1);
        var thisLoopArity = iterationVariables.checkAssignability(true);
        var actual = body.checkArity(0, expectedResultCount, thisLoopArity);
        return expectedResultCount;
      },
      codegenToTemporary: function (expr, context, tmp) {
  ...
  meta.KEY_ROOT_SCOPE.set('next', new meta.Symbol('next', 'builtin',
    new meta.SymbolData(meta.arityUnary, 'KEY', {
      ...
      codegenToTemporary: function (expr, context) {
        ...
        context.nextArgs.codegenParallelAssignments(context, expr.argAt(0));
  ...
  function Compiler(r) {
    ...
    Expr.prototype.checkAssignability = function (allowTuples) {
      ...
      if (this.sym === meta.tupleSymbol) {
        ...
        for (var i = 0; i < this.argCount(); i++) {
          if (!this.argAt(i).checkAssignability(false)) {
    ...
    Expr.prototype.codegenParallelAssignments = function (context, values) {
      if (this.sym === meta.tupleSymbol) {
        ...
        for (var i = 0; i < values.argCount(); i++) {
          ...
          var destExpr = this.argAt(i).codegenAsExpression(context);
        ...
        var v = values.codegenAsExpression(context);
        var d = this.codegenAsExpression(context);
        var s = meta.codegenAssignmentStatement(values.loc, '=', d, v);
