...
function Meta() {
  ...
  function Compiler(r, options) {
    ...
    Expr.prototype.postExpansionCompilationSteps = function () {
      ...
      debug('Before RESOLVE: %s', this.printAst());
    ...
    function AstPrinter(expr, indentationStep, lineLengthLimit) {
      if (!(this instanceof AstPrinter)) {
        return new AstPrinter(expr, indentationStep, lineLengthLimit);
      }

      this.indentationStep = typeof indentationStep !== 'undefined' ? indentationStep : '  ';
      this.lineLengthLimit = typeof lineLengthLimit !== 'undefined' ? lineLengthLimit : 120;
      this.expr = expr;
      this.children = [];
      for (var i = 0; i < expr.argCount(); i++) {
        this.children.push(
          new AstPrinter(expr.argAt(i), this.indentationStep, this.lineLengthLimit));
      }
    ...
    AstPrinter.prototype.appendChildren = function () {
      var junction = this.parentFitsLine ? ', ' : '\n';
      //var end = this.parentFitsLine ? '' : '\n';
      //var end = '';
      var result = '';
      for (var i = 0; i < this.children.length; i++) {
        if (i > 0) {
    ...
    AstPrinter.prototype.computeResult = function (prefix, mark) {
      if (typeof prefix === 'undefined') prefix = '';
      if (typeof mark === 'undefined') mark = '';
      var needsWrapping = false;
      var expr = this.expr;

      var argPrefix = prefix + this.indentationStep;
      if (expr.id() === 'if') {
        this.children[0].computeResult(argPrefix, this.fitsLine ? '' : '#cond ');
        this.children[1].computeResult(argPrefix);
        if (this.children.length > 2) {
          this.children[2].computeResult(argPrefix, 'else ');
        }
      } else if (expr.isCall() && !this.fitsLine) {
        this.children[0].computeResult('');
        this.children[1].computeResult(expr.argAt(1).isTuple() ? prefix : argPrefix);
      } else {
        for (var i = 0; i < this.children.length; i++) {
          this.children[i].computeResult(argPrefix);
        }
      }
        ...
        if (this.fitsLine) {
          ...
          if (expr.isCall()) {
            ...
            if (expr.argAt(1).isTuple()) {
              var tuple = this.children[1];
              for (var argIndex = 0; argIndex < tuple.children.length; argIndex++) {
                if (argIndex > 0) {
                  this.append('\n');
                }
                this.append(tuple.children[argIndex].result);
              }
            } else {
              this.appendChild(1);
            }
      ...
      this.prepend(mark);
      if (!this.parentFitsLine) {
        this.prepend(prefix);
      }


      //debug('   DONE: %s', expr.toExpressionString());
      //debug('       : %s', this.result);
