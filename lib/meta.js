...
function Meta() {
  ...
  var blockSymbolDo = new meta.BlockData(
    ...
    function (parser, firstChar) {
      ...
      if (parser.currentLineNumber === block.loc.start.line + 1) {
        ...
        if (block.argCount() > 0 && block.parent !== null && block.parent.id === '<line>') {
  ...
  meta.SymbolData = function (arity, precedence, options) {
    ...
    this.preExpand = meta.existsOrUndefinedFunction(options.preExpand);
    ...
    this.unquoteKeyScope = meta.existsOrNull(options.unquoteKeyScope);
  ...
  meta.callSymbol = new meta.Symbol('<call>', 'builtin',
    new meta.SymbolData(meta.arityBinaryRight, 'CALL', {
  ...
  meta.functionSymbol = new meta.Symbol('->', 'builtin',
    new meta.SymbolData(meta.arityBinary, 'FUNCTION', {
      ...
      expand: function (expr) {
        ...
        if (expr.argAt(1).isTuple() && expr.argAt(1).argCount() > 1) {
  ...
  meta.oldMetaSymbol = new meta.Symbol('meta', 'builtin',
    new meta.SymbolData(meta.arityUnary, 'KEY', {
      expand: function (expr) {
        if (expr.argCount() == 1 && expr.argAt(0).isTuple()) {
          expr.transformInto(expr.argAt(0));
  ...
  meta.addBuiltinKeySymbol(meta.oldMetaSymbol);

  meta.metaSymbol = new meta.Symbol('#meta', 'builtin',
    new meta.SymbolData(meta.arityUnary, 'KEY', {
      preExpand: function (expr) {
        var expander = expr.argAt(0).asEvalFunction(['ast'], false);
  ...
  meta.Error = function (message, line, column, originalLine, originalColumn, nestedErrors) {
  ...
  function Compiler(r, options) {
    ...
    this.options = options || meta.Meta.options;
    ...
    Expr.prototype.fromJsonObject = function (json, resolutionRoot, locationRoot, parent) {
      ...
      var unquoteKeyScope = resolutionRoot.unquoteKeyScope();
      var sym;
      switch (json.kind) {
        case 'builtin':
          ...
          break;
        case 'macro':
          sym = unquoteKeyScope.get(json.id);
          if (sym === null) {
    ...
    Expr.prototype.asTuple = function () {
      if (!this.isTuple()) {
        ...
        if (index >= 0) {
          this.parent.insertAt(index, this.newTuple(this));
        }
      }
    };
    ...
    Expr.prototype.newPlaceholder = function (value) {
      var result = new Expr(meta.placeholderSymbol, null, [], null);
      result.takeLocationFrom(this);
      return result;
    };
    ...
    Expr.prototype.keyScopeIsExtended = function () {
      return this.parent === null || this.keyScope !== this.parent.keyScope;
    };
    Expr.prototype.extendKeyScope = function () {
      if (!this.keyScopeIsExtended()) {
        this.keyScope = this.keyScope.extend();
      }
    };
    Expr.prototype.setupSubordinateKeys = function () {
      if (this.sym.symbolData.subordinate !== null) {
        this.extendKeyScope();
        var subordinate = this.sym.symbolData.subordinate;
        for (var i = 0; i < subordinate.length; i++) {
          this.keyScope.set(subordinate[i].id, subordinate[i]);
        }
      }
    };
    ...
    Expr.prototype.combineArguments = function (resultReceiver) {
      ...
      if (combined.sym !== meta.oldMetaSymbol) {
        combined.preExpand(resultReceiver);
      }
    ...
    Expr.prototype.defineSymbol = function (expr) {
      if (this.keyScope.has(expr.id)) {
        this.error('Redefined symbol ' + expr.id);
      } else {
    ...
    Expr.prototype.asFunctionBody = function (args, isVoid) {
    ...
    Expr.prototype.asFunction = function (args, isVoid) {
      var bodyCode = this.asFunctionBody(args, isVoid);
      var func = function () {};
    ...
    Expr.prototype.asEvalFunction = function (args, isVoid) {
      var bodyCode = this.asFunctionBody(args, isVoid);
    ...
    Expr.prototype.processMetaCommand = function () {
      ...
      if (callee.isTag()) {
        switch (callee.getTag()) {
          ...
          case 'macro':
            if (lowerArg.isCall()) {
              var secondCallee = lowerArg.argAt(0);
              var secondArg = lowerArg.argAt(1);
              var name = null;
              if (secondCallee.isValue()) {
                name = secondCallee.getValue();
                if (typeof name !== 'string') {
                  secondCallee.error('Invalid macro name');
                  return;
                }
              } else if (secondCallee.isTag()) {
                name = secondCallee.getTag();
              } else {
                secondCallee.error('Invalid macro name');
                return;
              }
              ...
              if (secondArg.isTuple()) {
                for (var i = 0; i < secondArg.argCount(); i++) {
                  if (!processMacroProperty(secondArg.argAt(i))) {
                    return;
                  }
                }
              } else {
                if (!processMacroProperty(secondArg)) {
                  return;
                }
              }
              ...
              var expandFunction = expand.asEvalFunction(['ast', 'meta'], false);
              this.keyScope.set(name, this.createMacro(name, arity, precedence, {
                dependent: dependent,
    ...
    Expr.prototype.processMetaKeyword = function () {
      if (this.sym !== meta.oldMetaSymbol) return;
    ...
    Expr.prototype.applyTransformer = function (
        transformerProperty, transformerName, applyFixers, resultReceiver) {
      var i = 0;
      while (i < this.argCount()) {
        var arg = this.argAt(i);
        var incrementIndex = arg.applyTransformer(transformerProperty, transformerName, applyFixers);
        if (incrementIndex) i++;
      }

      while (true) {
        var transformer = this.sym.symbolData[transformerProperty];
        var result = undefined;
        try {
          result = transformer(this, this.compiler.metaEnv);
        } catch (e) {
          this.error('Error in symbol ' + transformerName);
          this.error('error:\n' + e);
          this.error('stack:\n' + e.stack);
          this.error('JS:\n' + transformer.js);
          this.error('MJS:\n' + transformer.mjs);
        ...
        if (typeof result === 'undefined') {
          ...
          if (this.parent !== null) {
            ...
            return result.applyTransformer(transformerProperty, transformerName, applyFixers);
          } else {
    ...
    Expr.prototype.precombine = function (resultReceiver) {
      return this.applyTransformer('precombine', 'precombination', false, resultReceiver);
    };
    Expr.prototype.preExpand = function (resultReceiver) {
      return this.applyTransformer('preExpand', 'postcombination', false, resultReceiver);
    };
    Expr.prototype.expand = function (resultReceiver) {
      return this.applyTransformer('expand', 'expansion', true, resultReceiver);
    };
    ...
    Expr.prototype.createAlias = function (newId) {
      var oldSymbolData = this.sym.symbolData;
    ...
    Expr.prototype.createExtension = function (expander) {
      ...
      var result = this.createAlias(this.id);
      result.sym.symbolData.expand = expanderBuilder(this.sym.symbolData.expand, expander);
      return result;
    };

    Expr.prototype.unquoteKeyScope = function () {
      if (this.sym.symbolData.unquoteKeyScope !== null) {
        return this.sym.symbolData.unquoteKeyScope;
      } else {
        return this.keyScope;
      }
    };
    ...
    Expr.prototype.createMacro = function (id, arityName, precedence, options) {
      ...
      if (typeof preExpand !== 'function') {
        this.error('Invalid symbol preExpandr');
        return null;
      }
