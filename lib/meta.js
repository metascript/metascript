...
function Meta() {
  ...
  function Compiler(r, options) {
    ...
    Expr.prototype.normalizeLocation = function () {
      ...
      if (this.argCount() > 0) {
        var start = this.argAt(0).loc.start;
        var end = this.argAt(this.argCount() - 1).loc.end;
        this.loc.start.line = start.line;
        this.loc.start.column = start.column;
        this.loc.end.line = end.line;
        this.loc.end.column = end.column;
      }
    };
    Expr.prototype.includesLocation = function (line, column) {
      return (this.loc.start.line <= line) &&
          (this.loc.start.column <= column) &&
          (this.loc.end.line >= line) &&
          (this.loc.end.column >= column);
    };
    Expr.prototype.findLocation = function (line, column) {
      for (var i = 0; i < this.argCount(); i++) {
        var childResult = this.argAt(i).findLocation(line, column);
        if (childResult !== null) {
          return childResult;
        }
      }
      if (this.includesLocation(line, column)) {
        return this;
      }
      return null;
    };
